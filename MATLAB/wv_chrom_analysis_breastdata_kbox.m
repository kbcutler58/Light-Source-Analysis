...
wvlengths = [
    660 690 785 830;
%     688 830 915 976; Changed for kbox wavelengths
    678 830 915 976;
    683 830 915 976;
    693 830 915 976;
    698 830 915 976;
...
%             nfiles = floor((size(data,2)-2)); 
            end
            nfiles = length(header_index(header_index == 1))
...
%                 muas=data(indexes,j*2);
                ...
                concentrations = fdpmfit(j).phy(1,1:2);
...
%                    legend('new fit','old fit', 'mua')
            ...
            cd('C:\Users\Kyle\Downloads\DOSI data for wv LG testing\Breast Results 4wv new nonneg oxy')
