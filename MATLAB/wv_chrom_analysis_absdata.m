...
wvlengths = [
    ...
    testwvs=wvlengths(waves,:);
        ...
        folders = dir('Ab Data WV analysis');
        cd('Ab Data WV analysis');
...
%         path1 = 'C:\Users\Kyle\Downloads\DOSI data for wv LG testing\PROCESSED_use these processed data';
        path1='C:\Users\Kyle\Downloads\DOSI data for wv LG testing\Ab Data WV analysis';
        for k = 1:length(folders)-2
            ...
            for j=1:nfiles
                ...
                figure;
                hold on
                scatter(testwvs,fdpmfit(j).fitmua);
                scatter(testwvs,muas);
%                 scatter(testwvs,fdpmfit.fitmua,testwvs,muas);
                plot(wvs,data(:,j),wvs,fdpmfit2(j).fitmua);
                
            end
            OutputFile = strcat(folders(k+2).name,'_waves',num2str(waves,'%02d'),'_Abdo','.mat');
            cd('C:\Users\Kyle\Downloads\DOSI data for wv LG testing\Ab Results 4wv');
