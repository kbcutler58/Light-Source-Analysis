...
%             end
            ...
            cd('C:\Users\Kyle\Downloads\DOSI data for wv LG testing')
            fileoutputname = strcat('Patient',filename(1:16),'_ABS2_''meas.asc');
            ...
            fprintf(fid,'\n');
            for j=1:nfiles
                fprintf(fid,'\t%s',headers{1}{2*j});
            end
            fprintf(fid,'\n');
            for t = 1:4
                for j=1:nfiles
                fprintf(fid,'\t%f',DataTags(t,2*j));
                end
                fprintf(fid,'\n');
            end
            for i=1:length(fdpmchrom.names)
            ...
            fclose(fid);
            outputfile = strcat('wvanalysis_Feb24_5_results','.asc');            
            fid = fopen(outputfile,'a');
