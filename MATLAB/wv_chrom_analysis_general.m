...
%             cd(strcat(path1,'\',folders(k+2).name));
            ...
            elseif (strfind(D_mat6, 'Conc (SSFDPM):')) == 1
                broadband_fit_data_raw =dlmread(filename,'\t',[60 1 67 nfiles]);
            end
            ...
            broadband_fit_data=broadband_fit_data_raw(1:8,:);
            fclose(fid);
...
%                 PatientData.(PointLabel).muaSpectrum(4,:)=data_fit(:,j);
            ...
            OutputFile = strcat(folders(k+2).name,'_waves',num2str(waves,'%02d'),'_test','.mat');
            cd('C:\Users\Kyle\Downloads\DOSI data for wv LG testing');
